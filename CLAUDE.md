...
- 2× Hue White & Color Ambiance **B22** bulbs (model 9290024896; z2m labels them "E27" but they're the B22 twin pack), both onboarded 2026-06-28, `power_on_behavior=off`, RGB only (same `color_temp`-rejection gotcha as the E14s):
  ...
  - NB: this firmware **does** expose working Hue `effect`s over z2m (candle/fireplace/sunset/colorloop/etc.). Bedroom scenes = 5 distinct *static* RGB looks (Bright/Warm/Nightlight/Calm/Sunset) + a **Colour Cycle** (`colorloop`). **Colour Cycle is a `script` (`script.bedroom_colour_cycle`), not a scene**, because `colorloop` looks frozen on one colour unless `effect_speed` (0–1) is non-zero, and a scene can't set the `effect_speed` *number* entity — the script sets `number.bedroom_light_effect_speed`=0.5 then the effect. Dashboard has a "Cycle Speed" slider (`mushroom-number-card`) for live control. To clear an effect back to static, send `{"effect":"none"}` (the `effect` attribute otherwise lingers stale in HA even after a plain colour command).
