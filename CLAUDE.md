...
- 2× Hue White & Color Ambiance **B22** bulbs (model 9290024896; z2m labels them "E27" but they're the B22 twin pack), both onboarded 2026-06-28, RGB only (same `color_temp`-rejection gotcha as the E14s):
  - `bedroom_light` — IEEE `0x001788010daadc60` → HA `light.bedroom_light`. **`power_on_behavior=off`** on purpose: bedroom is HA-primary (sunrise/scenes/colour-cycle), so leave the wall switch ON and control via HA. A smart Zigbee wall button (controls the bulb *without* cutting power) is the planned physical fallback — until then, flipping the bedroom wall switch off makes the bulb unreachable.
  - `utility_light` — IEEE `0x001788010daadca8` → HA `light.utility_light`. Utility room by the front door (2nd fridge, pet food); dad's in/out transit point — **prime presence-sensor candidate** (like the LR POC), no sensor yet. **`power_on_behavior=on` (changed 2026-06-29)**: this room is wall-switch-primary, so flipping the switch must turn the bulb on. Caveat inherent to smart-bulb-behind-dumb-switch: while the switch is OFF the bulb is unpowered → offline/uncontrollable in HA until switched back on.
  - **The Hue native `effect`s (candle/fireplace/sunset/colorloop) do NOT animate on this bulb's firmware** (installed `16782358`). z2m accepts `{"effect":"colorloop","effect_speed":0.5}` and reports it, but the bulb's colour stays frozen on a single hue — verified via z2m logs (`color` x/y never changes) and visually, with `hue_native_control` both on and off. (Possibly needs the OTA firmware bump — `latest 16786690` available — but unconfirmed.) So **colour cycling is driven by HA, not the bulb**: automation `Bedroom Colour Cycle` steps `hs_color`/brightness while `input_boolean.bedroom_colour_cycle` is on (toggle on the dashboard), at `input_number.bedroom_cycle_speed` seconds/step (slider, lower=faster). `input_select.bedroom_cycle_mode` picks the animation — **Rainbow / Warm Glow / Ocean / Fire / Pastel / Party** (per-mode hue/sat/bri computed in the loop's `variables:` block); changing mode mid-cycle restarts the loop. `Bedroom Cycle Stop On Scene` auto-clears the toggle when a static scene is picked. Bedroom static scenes = 5 distinct RGB looks (Bright/Warm/Nightlight/Calm/Sunset). To clear a stuck `effect` attribute, send `{"effect":"none"}`.
...
- **HA configmap uses `subPath` mounts** for `configuration.yaml`, `scenes.yaml`, `automations.yaml`, `scripts.yaml`, `ui-lovelace.yaml`. ConfigMap updates do NOT propagate to the running pod with subPath mounts. After editing `applications/home-assistant/configmap.yaml`, restart the deployment. **Adding a NEW file key (e.g. `scripts.yaml`, 2026-06-28) also needs a matching `volumeMounts` entry in `deployment.yaml`** — subPath mounts each file individually, so an unmounted key simply won't exist in the pod and `!include` fails.
